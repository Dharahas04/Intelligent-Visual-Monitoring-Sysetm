...
FACE_DET_PROTO = os.path.join(MODEL_DIR, "deploy.prototxt")
FACE_DET_MODEL = os.path.join(MODEL_DIR, "res10_300x300_ssd_iter_140000.caffemodel")
MASK_MODEL_PATH = os.path.join(MODEL_DIR, "mask_detector_pretrained.h5")
...
def process_frame(
    ...
    frame = resize_to_width(frame, target_width=800)
...
def resize_to_width(frame: np.ndarray, target_width: int = 800) -> np.ndarray:
    h, w = frame.shape[:2]
    if w <= target_width:
        return frame
...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--no-display", action="store_true", help="Disable OpenCV windows for headless runs")
    return parser
...
def run_video(args: argparse.Namespace, face_detector: FaceDetector, mask_model: tf.keras.Model) -> None:
    ...
    if args.output:
        ...
        writer, output_codec = create_video_writer(args.output, fps, 800, int(h * (800 / max(1, w))))
    ...
    while True:
        ...
        frame_out, _, no_mask_count, _ = process_frame(
            frame,
            face_detector,
            mask_model,
            args.alert_threshold,
    ...
    print(json.dumps({
        ...
        "noMaskDetections": total_no_mask,
